...
- 单词模块 (Phase 2)
- 农场模块：首发六阶段与 farm-fix-01 全部完成；最终全仓 350/350，通过并推送至 `origin/main`
- 超市模块 (待规划)
- 窗口边框攀爬 (Phase 2)
- 模块错误隔离 (Phase 3)
...
- ✅ farm-04 已以 `64f049c` 提交到本地 `main`，未 merge/push。

## 🌾 农场经营 — farm-05 统一库存接入（2026-07-26）

- 📝 用户确认由 Codex 执行高难度单窗口 farm-05；锁定 `inventory` 为唯一可写库存，旧 `foodInventory` 只作为迁移输入并在迁移结果中删除。
- 📝 仓库出售/销毁与商店购买使用通用数量 overlay；普通食物和种子均支持批量购买，锁定种子可见置灰并显示农场等级，投喂仍固定消耗一件。
...
- ✅ farm-fix-01 已以 `8c8783e` 提交；2026-07-29 随农场完整提交链推送至 `origin/main`。

## 🌾 农场经营 — ARCH-10 最终收尾（2026-07-29）

- ✅ farm-01～06、farm-fix-01、架构设计与六份实施计划全部完成并有本地提交。
- ✅ `main` 与 `origin/main` 已同步至 `8c8783e`；推送后工作区干净。
- ✅ `PROJECT_BRIEF.md`、`docs/progress.md`、`docs/session-log.md` 已审计并清除当前状态中的“待提交、待复验、领先远端”等过期描述；历史窗口条目保留其交付当时状态。
- ✅ 当前没有未完成的农场实现窗口；ARCH-09 未分配任务，不构成待交付项。

## 🎨 农场视觉品质升级 — ARCH-11 设计与计划（2026-07-29）

- ✅ 用户确认固定镜头 `4×4` 场景、圆润明亮高质量 2D 卡通、PixiJS 场景 + DOM 经营面板、加工/订单双入口与统一换肤、AI 原创素材、中等克制动效和奶油星团纯视觉陪伴。
- ✅ Godot 作为长期待定项；当前只保留纯快照、用户意图、生命周期和通用资产格式，不引入 Godot/Unity 工程或桥接。
- ✅ 设计文档与八任务实施计划已完成；计划顺序为垂直样张 → manifest/快照 → Pixi 技术原型 → 完整场景 → UI 接入 → 加工订单换肤 → 跨页图标 → 最终复验。
- ⚠️ 技术原型必须先验证无 bundler Electron 项目在开发态与打包态的 Pixi ESM 入口解析；失败时停手上报最小 bundler 或受控 vendor 两种方案。
...
- ✅ 约 4.0 MiB 资产、视觉升级设计、八任务实施计划与 ARCH-11 追踪记录已作为同一阶段里程碑纳入 `main` 并推送至 `origin/main`。
...
- ✅ `farm-visual-03` 完成 Task 3：精确安装 `pixi.js@8.19.0`，建立唯一 ESM runtime boundary、固定六层 Pixi 垂直样张、9 critical/5 optional 资源分类、静态背景 + DOM hit targets 以及 Pixi → static → DOM 三级回退；尚未接入现有农场 UI。
...
- ✅ `farm-visual-04` 完成完整 Pixi 场景运行时：严格 `4×4` 纯布局、land/crop/building/pet/bird 对象工厂、snapshot 键控 reconciliation、按需纹理加载与 in-flight 去重、稳定深度排序、局部动画和完整 cleanup/竞态保护；该阶段只交付运行时，正式 UI 接入由后续 `farm-visual-05` 完成。
...
- ✅ `farm-visual-05` 完成正式 UI 接入：`farm-module` 组装生产 scene runtime，`farm-ui` 持有单一 persistent host，并以现有 VM → 深冻结 snapshot 驱动完整 Pixi 场景；Canvas 与 DOM 共用五类意图处理和既有 FarmService 命令，成功事务后播放定位反馈，视觉失败不影响业务结果。
