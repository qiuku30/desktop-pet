...
- 单词模块 (Phase 2)
- 农场模块：farm-01 库存与状态基础已验收；farm-02 领域引擎已通过 ARCH-10 第三轮独立复验、尚未提交；farm-03～06 待逐阶段授权与分窗
- 超市模块 (待规划)
- 窗口边框攀爬 (Phase 2)
- 模块错误隔离 (Phase 3)
...
- ✅ ARCH-10 第三轮独立复验通过：对抗迁移探针覆盖不完整 crop、非法订单、多个 running 加工任务及二次迁移幂等；7 个生产文件语法检查、纯模块禁用 API 扫描、`git diff --check` 均通过；新鲜全仓测试 254/254。farm-02 当前未 commit/merge/push。
