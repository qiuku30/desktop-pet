...
农场经营按六阶段计划推进：farm-01～04 已提交并验收，farm-04 提交为 `64f049c`；farm-05 统一库存接入经 tooltip 最小返修后已通过 ARCH-10 第二轮独立复验，当前改动未提交；farm-06 尚未立项。
...
|------|------|
| 📝 英语单词 | spec 占位，待细化需求 |
| 🌾 农场经营 | ARCH-10 已确认首发需求与架构设计；farm-01～04 已提交验收，farm-05 统一库存接入已通过第二轮独立复验、待用户授权提交 |
| 活动监视 | 设计文档已完成，隐私敏感 |
| 躲避光标 | 搁置，需主进程侧方案 |
...
**当前状态**：设计已审核并以 `5da6956` 提交，六阶段实施计划以 `2f6dd8a` 提交；`farm-01` 以 `05956a8` 提交并经 ARCH-10 两轮独立复验，全仓 211/211 通过。`farm-02` 经三轮独立复验、全仓 254/254 后以 `ad0da91` 提交。`farm-03` 完成农田/土地/建筑 UI、Dashboard 动态模块接入及可测试导航协调器；两轮返修后，第三轮失败链探针、全仓 277/277、语法/差异检查和 Electron 冷启动与页面往返通过，并以 `550aa3a` 提交。`farm-04` 首轮复验发现 settlement 补跑丢失及 settlement/mutation 并发竞态，返修后以可控 Promise 覆盖双向交错与 cleanup，定向 27/27、全仓 290/290、语法/差异检查及 Electron 加工/订单/确认框/页面往返均通过，以 `64f049c` 提交。上述提交均未 push。`farm-05` 首轮 tooltip 兼容性回归已最小返修；ARCH-10 第二轮独立复验确认目录元数据、真实渲染 helper 与生产调用链一致，定向 13/13、全仓 318/318、语法和差异检查通过，Electron 冷启动正常；受无边框透明窗辅助控制限制，真实鼠标悬停未能自动触发，字段、数值和高度由行为测试覆盖。farm-05 当前未 commit/merge/push，farm-06 尚未立项。
