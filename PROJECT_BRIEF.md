...
> 最后更新：2026-08-07（ARCH-09 完成 📝 英语单词模块全部设计与 Phase 1 交付）
> Phase 1、番茄钟、2048、桌宠形象化 Phase 2 与农场首发均已交付；单词模块共享层+面板+单词本可用。
...
| `docs/superpowers/specs/2026-07-30-farm-ui-scene-integration-design.md` | 农场 Pixi 场景正式 UI 接入、三级回退与响应式交互设计 |
| `docs/superpowers/plans/2026-07-30-farm-ui-scene-integration.md` | 农场场景 UI 接入严格 TDD 实施计划 |
...
Phase 1 宠物核心系统 + 番茄钟 + 2048 + 桌宠形象化 Phase 2 全部完成。面板八页（主页/仓库/商店/番茄/农场/2048/单词/设置）全部可用。

### 🔨 进行中

无。农场视觉升级由 ARCH-11 负责，`farm-visual-07` 已完成集成，待 `farm-visual-08` 最终综合复验。

### ⏳ 待实现

| 任务 | 说明 |
|------|------|
| 🎨 农场视觉升级 | ✅ Task 1～6、`farm-art-04` 与 `farm-visual-07` 已完成并集成；待执行 `farm-visual-08` 最终综合复验 |
| 📝 英语单词 Phase 2 | 宠物联动（word-04）、填空题、FSRS 参数拟合、模糊搜索纠错（Levenshtein） |
| 活动监视 | 设计文档已完成，隐私敏感 |
| 躲避光标 | 搁置，需主进程侧方案 |
...
单词（Phase 2）、农场后续扩展、超市（待规划）；桌宠形象化的多形态、外部皮肤包和用户照片生成属于长期扩展。
...
| 分支 | 状态 | 说明 |
|------|------|------|
| `main` | ✅ 本地稳定，领先远端 | 农场完整资产 `c3725bc`、完整 Pixi 场景运行时 `0d60c0b`、正式 UI 接入 `c6b1204` 已集成；尚未 push |
...
```
main 分支，23 commits 领先 origin/main（未推送）
工作区干净，37 测试全部通过
...
- [x] **推送 GitHub**：网络恢复后 `git push origin main`（已由 ARCH-07 完成）
- [x] **删除远程残留分支**：`origin/feature/shared-event-bus-pet-state`、`origin/feature/pet-movement`（已由 ARCH-07 完成）
- [x] **删除本地残留分支**：`feature/pet-movement`（已由 ARCH-04 完成）
...
**阻塞项**（已于 ARCH-07 解决）：网络恢复，已 push 所有 commits 并清理远程残留分支
...
## 2026-07-24 — ARCH-08 桌宠形象化设计确认

**处理事项**：重新评估当前主流桌宠的精灵帧、Live2D/Spine 和 3D 路线，细化桌宠形象化需求与实现边界。

**用户确认**：
1. 首期采用高清手绘 Q 版，不采用像素、Live2D 或 3D。
2. 首发角色为原创“奶油星团”：团子兽基础轮廓 + 少量星灵特征。
3. 首期单形态，架构预留成长与进化形态。
4. 首期 7 类动画：idle/walk/eat/happy/sad/interact/sleep。
5. 素材源使用独立透明 PNG/WebP 帧 + JSON 清单。
6. 运行时使用 Canvas；业务层、动画控制器、帧渲染器分离。
7. 等级和养成进度跨皮肤、跨形态共享。
8. 用户上传照片生成桌宠作为长期方向，本期只预留标准皮肤包协议。
...
- 最终自动验证为全仓 194/194，通过 Electron 冷启动、页面往返和 75/100/125/150% 缩放复验；全部实现均已 commit 并随 `main` 推送。
...
**当前状态**：设计已审核并以 `5da6956` 提交，六阶段实施计划以 `2f6dd8a` 提交；`farm-01` 以 `05956a8` 提交并经 ARCH-10 两轮独立复验，全仓 211/211 通过。`farm-02` 经三轮独立复验、全仓 254/254 后以 `ad0da91` 提交。`farm-03` 完成农田/土地/建筑 UI、Dashboard 动态模块接入及可测试导航协调器；两轮返修后，第三轮失败链探针、全仓 277/277、语法/差异检查和 Electron 冷启动与页面往返通过，并以 `550aa3a` 提交。`farm-04` 首轮复验发现 settlement 补跑丢失及 settlement/mutation 并发竞态，返修后以可控 Promise 覆盖双向交错与 cleanup，定向 27/27、全仓 290/290、语法/差异检查及 Electron 加工/订单/确认框/页面往返均通过，以 `64f049c` 提交。`farm-05` 首轮 tooltip 兼容性回归已最小返修；ARCH-10 第二轮独立复验确认目录元数据、真实渲染 helper 与生产调用链一致，定向 13/13、全仓 318/318、语法和差异检查通过，Electron 冷启动正常；受无边框透明窗辅助控制限制，真实鼠标悬停未能自动触发，字段、数值和高度由行为测试覆盖，并以 `33471a0` 提交。`farm-06` 首轮加工摘要旧键回退及小鸟异步领取上限竞态已严格 TDD 返修；ARCH-10 第二轮新鲜定向 51/51、全仓 349/349、语法/合规/差异检查通过，Electron 冷启动显示正式 Canvas 且未见 Emoji 闪变，并以 `564a3b5` 提交。`farm-fix-01` 修正 `.farm-tab-content` 缺少纵向 flex 导致的工具栏横排、网格压缩和操作区右溢；真实 Chromium 在 800×600、600×400 验证农田、加工、订单三页签无横向溢出，ARCH-10 独立复验定向 26/26、全仓 350/350，通过并以 `8c8783e` 提交。上述提交均已推送到 `origin/main`。

## 2026-07-29 — ARCH-11 农场视觉品质升级设计

**处理事项**：针对现有农场“Emoji 与文字拼接、缺少完整美术”的观感，与用户确认视觉方向、场景范围、素材路线、动效强度、首轮范围和长期引擎边界。

**已确认**：
1. 固定镜头完整展示 `4×4` 农田、农舍、水渠、围栏和少量环境装饰。
2. 采用圆润明亮、带体积与统一光影的原创高质量 2D 卡通，不采用像素、3D 或第三方游戏素材。
3. 首轮使用 PixiJS 场景 + DOM 经营面板；现有 FarmService/PetState/EventBus 继续作为业务权威。
4. 加工台、订单板使用场景建筑与顶部页签双入口；加工和订单同步统一换肤。
5. AI 辅助原创素材先通过垂直样张门槛，再批量生产；无法保证统一透视/光源时必须上报外部 2D 美术需求。
6. 奶油星团作为纯视觉陪伴，不自动劳作；中等克制动效继续支持减少动态效果。
7. Godot 只列长期待定；自由大地图、自由摆放、宠物寻路劳作等需求正式立项后再评估迁移。
8. 回退链锁定为完整 Pixi 场景 → 静态场景 + DOM → 现有可操作 DOM 农场。

**文档**：
- `docs/superpowers/specs/2026-07-29-farm-visual-upgrade-design.md`
- `docs/superpowers/plans/2026-07-29-farm-visual-upgrade.md`

**当前状态**：农场视觉升级 Task 1～6 已完成并集成；最新主线验证为 GUI 全仓 463/463 与 darwin/arm64 Forge package。下一步由 `farm-visual-08` 执行最终综合复验；尚未创建该窗口。上述本地视觉升级提交尚未 push。

---

## 2026-08-05~07 — ARCH-09 📝 单词模块立项、设计与全部交付

**处理事项**：选定英语单词为 Phase 2 第二个游戏模块，调研市面方案（SRS/游戏化/被动学习三流派），逐项确认设计，锁定 FSRS 算法 + ECDICT SQLite + L2 桌宠耦合方案。

**调研决策（9 项）**：
1. SRS → 选 FSRS（非 SM-2），默认参数启动后用真实数据拟合
2. 词库 → ECDICT + SQLite + better-sqlite3，运行时 IPC 查询
3. 学习判定 → 新词翻卡自评 / 复习选择题（熟词→填空题 Phase 2）
4. 每日上限 → 改为每日目标，达标点亮打卡，不限量
5. 入口 → 学习和复习各自独立入口
6. 答题流程 → 选项直接展示，答错/忘→排到队尾→必须答对
7. 搜索 → 英文前缀 + 中文包含双搜 + 输入联想下拉
8. 单词本 → 主页（全局搜索 + 我的 + 词库）+ 列表（字母索引 + 筛选）+ 详情
9. 桌宠耦合 → L2 偏 L3，Phase 2 实现

**拆窗方案（全部交付）**：

| 窗口 | 内容 |
|------|------|
| word-01 | FSRS 算法 + SQLite IPC + 词库索引 + store + events + ECDICT 提取脚本 |
| word-01 续 | scheduleReview 字段保留修复 |
| word-02 | 面板页面（首页 + 学习 + 复习 + 设置） |
| word-03 | 单词本（主页 + 列表 + 详情 + 搜索 + 收藏/重新学习） |
| word-fix-01 | 设置独立页面 + 中文释义优先 + 首页查词 |
| ARCH-09 自修 | initialS 0.5→0.001、上限→目标、不太熟排队尾、复习直接出选项、答错排队、答对也展卡、搜索升级 + 当前词库显示 |

**新增文件**：`specs/word-game.md`、`docs/word-01-prompt.md`

**当前全局状态**：
- 单词模块 Phase 1 全部交付（学习/复习/单词本/搜索完整可用）
- 待 Phase 2：宠物联动、填空题、FSRS 参数拟合、模糊纠错
- 未 commit（单词模块所有改动在本地工作区）
- 🟡 FSRS w 参数非真实社区拟合值（Phase 2 解决）
