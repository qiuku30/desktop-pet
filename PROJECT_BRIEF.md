...
农场经营按六阶段计划推进：farm-01、farm-02、farm-03 已提交并验收；farm-04 加工台与订单 UI 经竞态返修后已通过 ARCH-10 第二轮独立复验，尚未提交；farm-05～06 尚未立项。
...
|------|------|
| 📝 英语单词 | spec 占位，待细化需求 |
| 🌾 农场经营 | ARCH-10 已确认首发需求与架构设计；farm-01～03 已提交验收，farm-04 加工台与订单 UI 已验收、待提交 |
| 活动监视 | 设计文档已完成，隐私敏感 |
| 躲避光标 | 搁置，需主进程侧方案 |
...
**当前状态**：设计已审核并以 `5da6956` 提交，六阶段实施计划以 `2f6dd8a` 提交；`farm-01` 以 `05956a8` 提交并经 ARCH-10 两轮独立复验，全仓 211/211 通过。`farm-02` 经三轮独立复验、全仓 254/254 后以 `ad0da91` 提交。`farm-03` 完成农田/土地/建筑 UI、Dashboard 动态模块接入及可测试导航协调器；两轮返修后，第三轮失败链探针、全仓 277/277、语法/差异检查和 Electron 冷启动与页面往返通过，并以 `550aa3a` 提交。上述提交均未 push；`farm-04` 首轮复验发现 settlement 补跑丢失及 settlement/mutation 并发竞态，返修后以可控 Promise 覆盖双向交错与 cleanup，定向 27/27、全仓 290/290、语法/差异检查及 Electron 加工/订单/确认框/页面往返均通过，尚未提交。farm-05～06 尚未立项。
