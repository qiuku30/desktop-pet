...
农场经营按六阶段计划推进：farm-01 已提交并验收，farm-02 已完成第三轮 ARCH-10 独立复验，等待用户决定提交；farm-03～06 尚未立项。
...
|------|------|
| 📝 英语单词 | spec 占位，待细化需求 |
| 🌾 农场经营 | ARCH-10 已确认首发需求与架构设计；farm-01 已验收，farm-02 领域引擎已通过第三轮独立复验，farm-03～06 待立项 |
| 活动监视 | 设计文档已完成，隐私敏感 |
| 躲避光标 | 搁置，需主进程侧方案 |
...
**当前状态**：设计已审核并以 `5da6956` 提交，六阶段实施计划以 `2f6dd8a` 提交；`farm-01` 以 `05956a8` 提交并经 ARCH-10 两轮独立复验，全仓 211/211 通过。`farm-02` 已完成配置、schema v1、纯规则、加工/订单、离线结算与事务协调器；两轮审查返修后，ARCH-10 第三轮独立复验的对抗迁移探针、生产文件语法检查和纯模块边界检查均通过，全仓测试 254/254。farm-02 及其补充事实来源当前未提交、未 merge、未 push；farm-03～06 尚未立项。
